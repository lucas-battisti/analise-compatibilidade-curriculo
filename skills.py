...
""",
    ...
    response = chain.run(
        ...
        hard_skills_requirements=json.dumps(
            hard_skills_requirements, ensure_ascii=False
        ),
        soft_skills_requirements=json.dumps(
            soft_skills_requirements, ensure_ascii=False
        ),
...
def compute_skills(info_dict: List[dict]):
    ...
    req_weights = np.array(
        []
    )  # Guarda os pesos das habilidades (0.8 se for requisito, 0.2 se desejável)
    type_array = np.array(
        []
    )  # Guarda os tipos das habilidades (hard_skill, soft_skill, language)
    ...
    for i in info_dict:
        ...
        if i["mentioned_in_resume"] == 3:
            pontos_fracos_list.append(
                "Candidato não possui conhecimento de {}".format(i["name"])
            )
            perguntas_list.append(
                "Você tem alguma experiência com {}?".format(i["name"])
            )
            ...
            if i["level_comparison"] == -1:
                pontos_fracos_list.append(
                    "O candidato possui conhecimento de {} em nível inferior ao exigido".format(
                        i["name"]
                    )
                )
                b += -1
            ...
            # Se o nível for superior ao exigido
                ...
                pontos_fortes_list.append(
                    "O candidato possui conhecimento de {} em nível superior ao exigido".format(
                        i["name"]
                    )
                )
                b += 1
            ...
            # Nível igual ou não especificado
                ...
                pontos_fortes_list.append(
                    "O candidato possui conhecimento de {}".format(i["name"])
                )

                # Se o nível não for especificado e não for uma soft skill, sugerir pergunta
                if not i["level_comparison"] and i["type"] != "soft_skill":
                    perguntas_list.append(
                        "Qual seu nível de experiência com {}?".format(i["name"])
                    )

            # Se a habilidade foi mencionada em alguma experiência prática
            if i["mentioned_in_experience"] == True:
                pontos_fortes_list.append(
                    "Utilizou {} em suas experiencias".format(i["name"])
                )
                b += 2  # bônus adicional

        # Atualiza arrays para cálculo dos escores por tipo de habilidade
        type_array = np.append(type_array, i["type"])
        score_array = np.append(
            score_array, (a + b)
        )  # escore final é soma do base com bônus

        # Cálculo dos escores ponderados por tipo
        hard_skill_score = (
            np.average(
                score_array[type_array == "hard_skill"],
                weights=req_weights[type_array == "hard_skill"],
            )
            if np.any(type_array == "hard_skill")
            else 0.0
        )
        soft_skill_score = (
            np.average(
                score_array[type_array == "soft_skill"],
                weights=req_weights[type_array == "soft_skill"],
            )
            if np.any(type_array == "soft_skill")
            else 0.0
        )
        language_score = (
            np.average(
                score_array[type_array == "language"],
                weights=req_weights[type_array == "language"],
            )
            if np.any(type_array == "language")
            else 0.0
        )
