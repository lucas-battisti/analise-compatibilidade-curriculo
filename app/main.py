...
async def analisar_curriculo(
    ...
    if len(education_requirements) != 0:
        education_info = evaluate_education_requirements(
            education_requirements, profile_json
        )
    ...
    if len(experience_requirements) != 0:
        experience_info = evaluate_experience_requirements(
            experience_requirements, profile_json
        )
    ...
    skills_info = evaluate_skill_requirements(
    ...
    return JSONResponse(
        {
            "Escores": {
                ...
                "Educação": (
                    round(education_score, 2) if len(education_requirements) != 0 else 0
                ),
                "Experiência": (
